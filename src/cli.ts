...
async function asyncMain(filePath: string, options: any) {
    ...
    async function processMultiview(moduleNames: string[]) {
        // First, render all modules to STL files with lower quality for faster multiview
        ...
        for (const moduleName of moduleNames) {
            ...
            if (options.dry) {
                ...
                try {
                    ...
                    // File doesn't exist, render it with lower quality for multiview
                    console.log(`Rendering ${moduleName} to ${stlFile} for multiview (using $fn=32 for speed)...`);
                    
                    // Generate temp file
                    const tempPath = tmp.tmpNameSync({ postfix: ".scad" });
                    const tempFileContents = `// Generated by scadr for multiview\nuse <${fullPath}>\n${moduleName}();`;
                    await fs.writeFile(tempPath, tempFileContents, { encoding: "utf-8" });
                    
                    // Override $fn via command line parameter
                    const args = [
                        `--o`, stlFile,
